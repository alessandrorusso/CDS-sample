...
import it.leonardo.localapp.config.KafkaProperties;
...
class LocalAppKafkaResourceIT {
    ...
    void setup() {
        ...
        LocalAppKafkaResource kafkaResource = new LocalAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/local-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/local-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
